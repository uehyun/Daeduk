...
public class ExceptionController {
	...
	/*
		...
		6. 예외처리 어노테이션
		- @ExceptionHandle와 @ControllerAdvice를 이용하여 처리
		...
		- web.xml 설정을 통해 처리할 수 있다.
			...
			예외처리 방법
			- 웹컨테이너 설정
				> 404 에러를 처리할 수 있도록 DispatcherServlet의 throwExceptionIfNoHandlerFound 속성을 true로 설정
		...
		- @Validated 어노테이션을 사용하면 Bean Validation의 유효성 검증 매커니즘을 이용할 수 있다.
			...
			- 입력값 검증 기능의 활성화와 BindingResult 정의
				> 입력값 검증 대상인 자바빈즈 메소드 매개변수에 @Validated 어노테이션을 지정하고 바로 다음에 BindingResult를 정의
				BindingResult에는 요청 데이터의 바인딩 에러와 검사 에러 정보가 저장
	*/
