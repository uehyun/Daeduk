...
public class ValidationController {
		...
		[환경설정]
		 ...
		 - 이때, 규칙을 활성화하기 위해서 사용할 어노테이션이 있다.
		 	...
		 	> 입력값 검증 대상의 도메인 클래스 직후에 BindingResult를 정의
		 		> BindingResult에는 요청 데이터의 바인딩 오류와 입력값 검증 오류 정보가 저장
		 
		 # 입력값 검증 환경설정 순서
		 1. 입력값 검증을 위한 의존 라이브러리를 추가
		 2. 입력값 검증 활성화
		 	> 활성화를 할 도메인 클래스안에 @Validated 어노테이션을 지정
		 3. 도메인 클래스 내 필드에다가 검증을 위한 어노테이션들로 데이터 검증을 설정(@NotBlank, @Size 등등)
		 4. 에러를 받을 BindingResult를 설정한다. (컨트롤러 메서드 내에 설정)
	*/
	...
	*/
	...
	/*
		3. 입력값 검증 규칙
		- 입력값 검증 규칙은 Bean Validation이 제공하는 제약 어노테이션으로 설장한다.
		
			검사 규칙은 크게 다음 세가지로 분류할 수있다.
			- Bean Validtaion 표준 제약 어노테이션
			- 서드파티에서 구현한 제약 어노테이션 (서드파티란 제 3자에서 만든...)
			- 직접 구현한 제약 어노테이션
			
			1) Member 클래스에서 테스트를 위한 어노테이션으로 설정(아래 명시되어 있는 내용들을 가지고 테스트)
			...
			@Length(max=, min=)		: 문자열의 길이가 min과 max 사이인지 확인합니다.
			
			[테스트]
			- Member 클래스의 검증 활성화 추가
				> userId, password, userName, email, dateOfBirth
			- 테스트 URL : /validation/registerValidationForm02로 요청해서 테스트를 진행
