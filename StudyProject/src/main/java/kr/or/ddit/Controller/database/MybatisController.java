	...
	/*
		...
		- 인터페이스의 구현을 mybatis-spring에서 자동으로 생성할 수 있다.
			...
			1) 마이바티스 구현
				...
				- 게시판 삭제 기능 컨트롤러 메소드 만들기 (curdRemove:post)
		...
		3. 별칭 적용
		- TypeAlias로 맵핑 파일에서 반복적으로 사용될 패키지의 이름을 정의
			...
			1) 마이바티스 설정
				...
				1-2) boardMapper_SQL.xml의 type의 설정을 별칭으로 설정
					- mybatisAlias가 설정되어 있지 않은 경우엔 타입으로 설정하고자 하는 타입 형태를 패키지명이 포함되어 있는 구조로 설정
					- 쿼리 태그에 각각 셋팅한 패키지명 대신 alias로 설정한 별칭으로 대체
		...
		6. 동적 SQL
		- 마이바티스는 동적 SQL을 조힙하는 구조를 지원하고 있으며, SQL 조립 규칙을 매핑 파일에 정의할 수 있다.
		...
		- 마이바티스 기능을 활용하여 매핑 파일을 적절하게 정의하면 일대다 관계 테이블 매핑을 쉽게 처리할 수 있다.
			...
			1) 게시판 구현 설명
				...
				- 회원 수정 화면 페이지 만들기 (crud/member/modifyForm.jsp)
				- 여기까지 확인
	...
	*/
	...
	/*
		...
		1. Tiles란?
			...
			- 하나의 화면들을 만들다보면 공통적이고 반복정으로 생성해야하는 header, footer와 같은 영역들이 존재합니다.
		...
		** 모듈화를 진행할 페이지
		...
		- footer.jsp (footer 영역에서 사용할 페이지입니다.)
			...
			그 외에 다양한 영역의 페이지는 구현하고자 하는 시나리오를 바탕으로 페이지가 구성될 때 추가적으로 레이아웃 영역을 분리하여
			작성하면됩니다.
		...
		2. Tiles 설정
			...
			1) Tiles를 사용하기 위한 의존 관계 등록
				...
				** 의존 관계를 등록 후 꼭 Project Update를 진행해서 라이브러리 등록을 진행
