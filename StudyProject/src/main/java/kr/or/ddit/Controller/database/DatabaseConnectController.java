	...
	/*
		...
		5. 스프링 JDBC
		 - SQL로만 데이터베이스를 쉽게 처리할 수 있도록 도와주는 JDBCTemplate 클래스를 제공한다.
		 	...
		 	3. 게시판 구현
		 		...
		 		기본적인 JDBC를 이용한 DB설계는 DAO란에서 sql 쿼리가 노출되어 개발하는 방식이다 보니 우리가 중급프로젝트때 또는 JSP에서
		...
		- JPA는 자바 표준 ORM입니다.
		 즉, 구현되어 있는 클래스에 테이블을 매핑하기 위한 프레임워크이다.
		 	...
		 	# JPA 관련 어노테이션
		 	자바빈즈 클래스 객체가 곧 테이블 구조와 같은 형태이기 때문에, VO클래스 내 @Entity어노테이션을 통해 해당 객체가 테이블과 같은 Entity임을
		 	설정하고 @Table어노테이션으로 테이블명을 설정해주면 VO클래스는 데이터 베이스의 생성되어 있는 테이블과 연결되기 위한 준비가 된다.
		 	ex)
		 		@Entity
		 		@Table(name="board")
		 		public class Board{...}
		 	그리고  각 필드(멤버변수)에 @Column(name="board_no") 어노테이션으로 테이블 컬럼과 매핑 설정을 한다.
		 	설정된 Entity등을 이용하여 jpa객체를 통해서 함수를 호출하면(persist, find 등등) 호출된 함수를 통해 Spring Data JPA가
		 	자동으로 Entity를 분석하여 함수에 따른 쿼리를 자동생성하여 결과를 실행, 조회, 수정 등을 처리해준다.
		 	기존 SQL 쿼리를 이용하여 서비스를 처리할 때에 필드가 추가되면 필드와 관련되어 있는 기능, sql, 추가된 필드 등을 수정해야 하는 번거로움이
		 	발생하지만 JPA는 필드만 추가 후 테이블 정보와 매핑만 시켜주면 JPA가 자동으로 분석을 진행한다.
		 	그리고, 사용하는 데이터베이스 종류가 변경된다 하더라도 문제가 없다. (JPA는 SQL을 직접 건들이지 않고 함수와 같은 기능들로만 쿼리를 요청하고
		 	데이터베이스와 통신하기 때문)
		 	
		 	JPA는 JPA구현체인 hibernate를 이용하여 사용, 그러기 위해선 관련 의존관계를 등록 후 사용해야 함
		 	SQL문을 이용하여 데이터베이스를 연동하여 데이터를 가공하는게 아닌, Method를 통해 데이터베이스를 조작할 수 있다는 장점이 있어 객체모델을
		 	이용하여 비즈니스 로직을 구성하는 데에만 집중을 할 수 있다. 하지만, 프로젝트의 규모와 복잡도가 클 때는 설계가 잘 못 되는 경우 해당 프로젝트의
		...
		*** 기업 면접에서 ORM은 뭐 쓰셨어요? 라는 질문을 받는 경우가 있다.
			- 저는 ibatis를 사용했습니다, 저는 mybatis를 사용했습니다 와 같은 대답
		---------------------------------
		
		7. Mybatis
		- SQL과 자바 객체를 매핑하는 아이디어로 개발된 데이터베이스 접근용 프레임워크이다.
		
		마이바티스 관련 내용은 12장에서 더 자세하게 알고 넘어가도록 합시다.
			...
			- 예제로 mapper패키지 안에 BoardMapper.xml을 구성하여 테스트를 진행
	*/
}
